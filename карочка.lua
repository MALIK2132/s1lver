-- 1. Отключение лишнего
pcall(function()
    game.Players.LocalPlayer.PlayerScripts.CharacterAndBeamMove.Enabled = false
end)

-- 2. Супер-движок интро
task.spawn(function()
    local Players = game:GetService("Players")
    local TS = game:GetService("TweenService")
    local lp = Players.LocalPlayer
    
    -- Пытаемся найти место, где звук ТОЧНО проиграется
    local soundParent = lp:FindFirstChild("PlayerGui") or game:GetService("SoundService")
    
    -- Создаем звук
    local sound = Instance.new("Sound")
    -- Я подобрал 3 максимально стабильных ID. Попробуй сначала этот:
    sound.SoundId = "rbxassetid://138420499694743" -- Классический "Sparkle/Magic" звук, он обычно везде разрешен
    sound.Volume = 0.5 -- На максимум
    ...
    if not success then sg.Parent = lp:FindFirstChild("PlayerGui") end

    local img = Instance.new("ImageLabel", sg)
    img.Size = UDim2.new(0, 320, 0, 320)
    img.Position = UDim2.new(0.5, 0, 0.35, 0)
    img.AnchorPoint = Vector2.new(0.5, 0.5)
    img.BackgroundTransparency = 1
    img.ImageTransparency = 1
    img.Image = "rbxassetid://138420499694743"

    local txt = Instance.new("TextLabel", sg)
    txt.Size = UDim2.new(1, 0, 0, 200)
    txt.Position = UDim2.new(0.5, 0, 0.47, 0)
    txt.AnchorPoint = Vector2.new(0.5, 0.47)
    txt.BackgroundTransparency = 1
    txt.Text = "I Love you kara 😘"
    txt.TextColor3 = Color3.fromRGB(255, 105, 180)
    txt.TextSize = 76
    txt.Font = Enum.Font.GothamBold
    txt.TextTransparency = 1

    -- СУПЕР-ФИШКА: Если обычный Play() не сработал
    task.wait(0.2)
    sound:Play()
    
    -- Если звук все равно молчит, попробуем форсированно обновить его
    sound.Playing = true 

    -- Анимация
    local ti = TweenInfo.new(1.2, Enum.EasingStyle.Quart)
    TS:Create(txt, ti, {TextTransparency = 0}):Play()
    TS:Create(img, ti, {ImageTransparency = 0}):Play()
    
    task.wait(4)
    
    TS:Create(txt, ti, {TextTransparency = 1}):Play()
    TS:Create(img, ti, {ImageTransparency = 1}):Play()
...
local function sendHubLoadedMessage()
	local message = " Owner Version | mur4kka loaded. "
	local sent = false
	pcall(function()
...
DefenseGroup:AddToggle("AntiKill", {
    Text = "Anti Loop Kill",
    Default = false,
    Callback = function(v)
        ...
        if v then
            _G.cons["antiloopkill"] = plr.CharacterAdded:Connect(function(char)
                ...
                if hrp then
                    -- Ждем буквально один кадр перед телепортом
                    RunService.RenderStepped:Wait()
                    
                    local target = CFrame.new(524.703979, 93.7120056, -375.040985)
                    hrp.CFrame = target
                    
                    -- Быстрая подстраховка на следующие 2 кадра
                    for i = 1, 2 do
                        RunService.RenderStepped:Wait()
                        hrp.CFrame = target
                    end
...
DefenseGroup:AddButton({
    Text = "Sequential Double-Reset",
    Func = function()
        local lp = game:GetService("Players").LocalPlayer
        local rep = game:GetService("ReplicatedStorage")

        -- Function to force-kill the current character
        local function killChar(character)
            if character then
                ...
                -- Clear any grab remotes to be safe
                local struggle = rep:FindFirstChild("Struggle", true)
                if struggle then struggle:FireServer() end
            end
        end

        -- 1. First Reset
        Library:Notify("First Reset...", 2)
        killChar(lp.Character)

        -- 2. Setup a "One-Time" listener for the next spawn
        local connection
        connection = lp.CharacterAdded:Connect(function(newCharacter)
            -- Disconnect immediately so it doesn't keep killing you forever!
            connection:Disconnect()
            
            -- Wait a tiny moment for the character to "stand up"
            task.wait(0.1) 
            
            -- 3. Second Reset
            killChar(newCharacter)
            Library:Notify("Double Reset Complete", 2)
        end)
    end,
    DoubleClick = false,
    Tooltip = "Kills you, waits for respawn, then kills you again instantly."
})
...
DefenseExtra:AddCheckbox("AutoGucciToggle", {
	Text = "Anti Gucci (Blobman) PRO",
Default = false,
Callback = function(Value)
    autoGucciActive = Value
    if Value then
        startAntiGucci()
        notify("System", "Anti-Gucci & Anti-Ragdoll Active", 3)
        ...
        task.spawn(function()
            ...
            while autoGucciActive do
                -- 1. Защита от Ragdoll (Падения/Оглушения)
                -- Отключаем состояния, которые заставляют персонажа падать
                Humanoid:SetStateEnabled(Enum.HumanoidStateType.Ragdoll, false)
                Humanoid:SetStateEnabled(Enum.HumanoidStateType.FallingDown, false)
                
                -- Если персонаж уже упал, принудительно поднимаем его
                ...
                if not blobExists then
                    stopAntiGucci()
                    spawnBlobman()
                    notify("System", "Blobman lost, respawning...", 2)
                    ...
                    if autoGucciActive and toysFolder and toysFolder:FindFirstChild("CreatureBlobman") then
                        startAntiGucci()
                        notify("System", "Blobman restored.", 2)
                    end
                end
                
                task.wait(0.1) -- Уменьшена задержка для более быстрой реакции Anti-Ragdoll
            end
        end)
    else
        ...
        -- Возвращаем стандартные состояния при выключении
        ...
        notify("System", "Anti-Gucci Disabled.", 3)
    end
end
})
...
DefenseExtra:AddCheckbox("AntiInputLag", {
    Text = "Anti Input Lag",
    Default = false,
    Callback = function(Value)
...
TargetGroup:AddDropdown("KickPlayerDropdown1", {
...
TargetGroup:AddButton({
    ...
    Func = function()
        ...
        Options.KickPlayerDropdown1:SetValues(list)
        Options.KickPlayerDropdown1:SetValue(nil)
...
TargetGroup:AddInput("KickHeightInput", {
...
-- Pre-calculate the payload to save memory
local arg = string.rep("⛓️", 1000000) 


BlobGroup:AddToggle("mur4kkaBwlobKickToggle", {
    Text = "Packets on Blob",
    Default = false,
    Tooltip = "Sends large data chunks to the server relay",

    Callback = function(Value)
        -- 'Value' is true when toggled on, false when off
        _G.SendPackets = Value
        
        task.spawn(function()
            while _G.SendPackets do
                -- Locating remotes inside the loop in case they respawn/change
                local grabevents = game.ReplicatedStorage:FindFirstChild("GrabEvents")
                local remote = workspace:FindFirstChild(plr.Name .. "SpawnedInToys")
                    and workspace[plr.Name .. "SpawnedInToys"]:FindFirstChild("CreatureBlobman")
                    and workspace[plr.Name .. "SpawnedInToys"].CreatureBlobman:FindFirstChild("BlobmanAnimations")
                    and workspace[plr.Name .. "SpawnedInToys"].CreatureBlobman.BlobmanAnimations:FindFirstChild("RelayClientAnimation")

                if grabevents then
                    local extend = grabevents:FindFirstChild("ExtendGrabLine")
                    local createg = grabevents:FindFirstChild("CreateGrabLine")
                    
                    if extend then extend:FireServer(arg) end
                    if createg then createg:FireServer(arg) end
                end
                ...
                -- Brief wait to prevent your own client from crashing
                task.wait(0.1) 
            end
        end)
    end
})
-- Combined Toggle for mur4kka blob kick
BlobGroup:AddToggle("mur4kkaBlobKickToggle", {
    Text = "mur4kka (grab + blob)",
    Default = false,
    Callback = function(on)
        kickLoopEnabled = on
        ...
        if on then
            ...
            if not selectedKickPlayer then
                ...
                if Toggles.mur4kkaBlobKickToggle then Toggles.mur4kkaBlobKickToggle:SetValue(false) end
                return
            end
...
BlobGroup:AddToggle("DualHandKillBlob", {
    Text = "mur4kka Kill Blob [Fast]", 
    Default = false,
    Callback = function(Value)
        ...
        if Value then
            ...
            if not tt or not tt.Character or not tt.Character:FindFirstChild("HumanoidRootPart") then
                ...
                Toggles.DualHandKillBlob:SetValue(false)
                return
            end
...
TargetGroup:AddToggle("EnableRagdollTarget", {
    Text    = "Ragdoll Target",
    Default = false,
    Callback = function(Value)
        local RS = game:GetService("ReplicatedStorage")
        ...
        if Value then
            if not selectedKickPlayer then
                ...
                Toggles.EnableRagdollTarget:SetValue(false)
                return
            end
            ...
            local function startPalletCache()
                ...
                getgenv().palletCacheConn = toysFolder.ChildAdded:Connect(function(child)
                    ...
                    if partOwner and partOwner.Value == lpName then
                        ...
                        task.spawn(function()
                            while getgenv().palletRagdollActive and child.Parent do
                                local tChar = selectedKickPlayer and selectedKickPlayer.Character
                                local tRoot = tChar and tChar:FindFirstChild("HumanoidRootPart")

                                if tRoot and soundPart.Parent then
                                    soundPart.CFrame = CFrame.new(tRoot.Position.X, tRoot.Position.Y + 23, tRoot.Position.Z)
                                    task.wait(0.02)
                                    soundPart.CFrame = CFrame.new(tRoot.Position.X, tRoot.Position.Y + 19, tRoot.Position.Z)
                                    soundPart.AssemblyLinearVelocity = Vector3.new(0, -200, 0)
                                end
...
Callback = function(v)
    ...
    task.spawn(function()
        ...
        local DestroyLine = GE:WaitForChild("DestroyGrabLine")
        local MenuToys = RS:WaitForChild("MenuToys")
        local SpawnRemote = MenuToys:WaitForChild("SpawnToyRemoteFunction")
        local DestroyToy = MenuToys:WaitForChild("DestroyToy")
        
        local verticalDistance = 20 
        local offsetVec = Vector3.new(0, verticalDistance, 0)

        while _G.applymethod and RunService.RenderStepped:Wait() do
            local tt = selectedKickPlayer
            if not tt then continue end
            ...
            local oldPos = HRP.CFrame -- Точка возврата
            local tChar = tt.Character
            local tHRP = tChar and tChar:FindFirstChild("HumanoidRootPart")
            
            if tChar and tHRP then
                local inv = workspace:FindFirstChild(LocalPlayer.Name .. "SpawnedInToys") or workspace:FindFirstChild(LocalPlayer.Name .. " SpawnedInToys")
                
                -- Если цель еще не захвачена
                if tChar.Parent == workspace and not tHRP:FindFirstChild("BodyPosition") then
                    -- МГНОВЕННЫЙ ЗАХВАТ
                    local targetCF = tHRP.CFrame
                    HRP.CFrame = targetCF * CFrame.new(0, 0, 2) -- Теп к нему
                    
                    local blob = inv and inv:FindFirstChild("CreatureBlobman")
                    if not blob then
                        pcall(function() SpawnRemote:InvokeServer("CreatureBlobman", targetCF, Vector3.zero) end)
                        task.wait(0.2) -- Минимальная задержка
                    ...
                    if blob and blob:FindFirstChild("VehicleSeat") then
                        blob.Name = "cringe"
                        -- Быстрая посадка
                        pcall(function() blob.VehicleSeat:Sit(hum) end)
                        
                        local scriptObj = blob:FindFirstChild("BlobmanSeatAndOwnerScript")
                        if scriptObj then
                            local CG = scriptObj:FindFirstChild("CreatureGrab")
                            local CD = scriptObj:FindFirstChild("CreatureRelease") or scriptObj:FindFirstChild("CreatureDrop")
                            
                            if CG and CD then
                                local rDetector = blob:FindFirstChild("RightDetector")
                                local t_start = tick()
                                repeat
                                    task.wait()
                                    local rWeld = rDetector and (rDetector:FindFirstChild("RightWeld") or rDetector:FindFirstChildWhichIsA("Weld"))
                                    if rWeld then
                                        pcall(function() CG:FireServer(rDetector, tHRP, rWeld) end)
                                        pcall(function() CD:FireServer(rWeld) end)
                                    end
                                until (tick() - t_start > 0.5) -- Сократил время для быстроты
                            end
                        end
                    end
                ...
                -- ЛОГИКА КИКА (УДЕРЖАНИЕ И РАГДОЛЛ)
                if tHRP and HRP then
                    -- Смена владельца сети (чтобы его не телепортировало назад)
                    pcall(function()
                        SetNetOwner:FireServer(tHRP, tHRP.CFrame)
                        DestroyLine:FireServer(tHRP)
                    end)

                    -- Усиленный рагдолл (Невидимая палетка)
                    if Toggles.EnableRagdollTarget and Toggles.EnableRagdollTarget.Value then
                        local pallet = getgenv().PalletForRagdoll
                        if pallet and pallet:FindFirstChild("SoundPart") then
                            pallet.SoundPart.Transparency = 1
                            if pallet.SoundPart:FindFirstChildOfClass("SelectionBox") then pallet.SoundPart:FindFirstChildOfClass("SelectionBox").Visible = false end
                            
                            -- Ставим палетку прямо в игрока с огромной скоростью
                            pallet.SoundPart.CFrame = tHRP.CFrame
                            pallet.SoundPart.AssemblyLinearVelocity = Vector3.new(0, 5000, 0) -- Мощный импульс
                        end
                    end

                    -- Силовое поле (BodyPosition)
                    if not kickbp or kickbp.Parent ~= tHRP then
                        ...
                        kickbp.P = 20000 -- Еще выше жесткость
                        kickbp.D = 100   -- Меньше демпфирования для резкости
                        kickbp.Parent = tHRP
                    end
                    
                    if not kickbg or kickbg.Parent ~= tHRP then
                    ...
                    -- Удержание на высоте 20 блоков
                    kickbp.Position = HRP.Position + offsetVec
                    
                    -- Анти-фриз: обнуляем скорость цели, чтобы она не "лагала" в воздухе
...
TargetGroup:AddToggle("RagdollKickToggle", {
Text = "norm kick",
Default = false,
Callback = function(v)
    _G.applymethod = v
    ...
    task.spawn(function()
        ...
        -- Настройка дистанции и направления (сверху на 20 блоков)
        local verticalDistance = 20 
        local offsetVec = Vector3.new(0, verticalDistance, 0)
        local offsetCF = CFrame.new(0, verticalDistance, 0)
        ...
        while _G.applymethod and RunService.RenderStepped:Wait() do
            ...
            if tChar and tHRP then
                ...
                -- Логика с Blobman (если игрок не в воркспейсе)
                if tChar.Parent ~= workspace then
                    local blob = inv and inv:FindFirstChild("CreatureBlobman")
                    if not blob then
                        pcall(function() SpawnRemote:InvokeServer("CreatureBlobman", HRP.CFrame * CFrame.new(5, 5, 20), Vector3.zero) end)
                        task.wait(0.5)
                        blob = inv and inv:FindFirstChild("CreatureBlobman")
                    end
                    if blob and blob:FindFirstChild("RightDetector") and blob:FindFirstChild("VehicleSeat") then
                        blob.Name = "cringe"
                        ...
                        if scriptObj then
                            ...
                            local CD = scriptObj:FindFirstChild("CreatureRelease") or scriptObj:FindFirstChild("CreatureDrop")
                            if CG and CD then
                                local bHRP = blob:FindFirstChild("HumanoidRootPart") or blob.PrimaryPart
                                if bHRP then bHRP.CFrame = tHRP.CFrame end
                                task.wait(0.2)
                                local t_start = tick()
                                repeat
                                    task.wait()
                                    local rWeld = blob.RightDetector:FindFirstChild("RightWeld") or blob.RightDetector:FindFirstChildWhichIsA("Weld")
                                    if rWeld then
                                        pcall(function() CG:FireServer(blob.RightDetector, tHRP, rWeld) end)
                                        pcall(function() CD:FireServer(rWeld) end)
                                    end
                                until (tick() - t_start > 1.5)
                                task.wait(0.2)
                                tHRP.CFrame = pos * offsetCF
                                HRP.CFrame = pos
                                pcall(function() DestroyToy:FireServer(inv:FindFirstChild("cringe")) end)
                                task.wait(0.5)
                            end
                ...
                if tHRP and hum and HRP then
                    -- Смена владельца сети (Network Ownership)
                    pcall(function()
                        for i = 1, 4 do
                            DestroyLine:FireServer(tHRP)
                            RunService.RenderStepped:Wait()
                    ...
                    -- Рагдолл с невидимой паллеткой
                    task.spawn(function()
                        if Toggles.EnableRagdollTarget and Toggles.EnableRagdollTarget.Value then
                            local pallet = getgenv().PalletForRagdoll
                            if pallet and pallet:FindFirstChild("SoundPart") then
                                -- Делаем паллетку невидимой
                                pallet.SoundPart.Transparency = 1
                                if pallet.SoundPart:FindFirstChildOfClass("SelectionBox") then
                                    pallet.SoundPart:FindFirstChildOfClass("SelectionBox").Visible = false
                                end
                    ...
                    -- Удержание игрока сверху через BodyPosition
                    if not kickbp or kickbp.Parent ~= tHRP then
                        if kickbp then kickbp:Destroy() end
                        kickbp = Instance.new("BodyPosition")
                        kickbp.Parent = tHRP
                        kickbp.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
                        kickbp.D = 200
                        kickbp.P = 10000 -- Увеличена жесткость для точности
                    end
                    ...
                    -- Обнуление велосити для фиксации положения
...
TargetGroup:AddToggle("ApplyMethodGrab", {
    Text = "Apply Method Grab",
    Default = false,
    Callback = function(v)
        _G.applymethod = v
        local kickbp, kickbg
        ...
        task.spawn(function()
            ...
            while _G.applymethod and RunService.RenderStepped:Wait() do
                ...
                local pos = HRP.CFrame
                local offsetCF = CFrame.new(0, 5, -5)
                local offsetVec = Vector3.new(0, 5, -5)
                ...
                if tChar and tHRP then
                    ...
                    if tHRP and hum and HRP then
                        ...
                        if (tHRP.Position - HRP.Position).Magnitude >= 29 and tChar.Parent == workspace then
                            task.wait(0.1)
                            HRP.CFrame = tHRP.CFrame
                            task.wait(0.2)
                            pcall(function() SetNetOwner:FireServer(tHRP, tHRP.CFrame) end)
                            task.wait()
                            HRP.CFrame = pos
                            task.wait(0.2)
                            for i,v in pairs(tChar:GetChildren()) do
                                if v:IsA("BasePart") and v.Name ~= "Humanoid" then
                                    v.CFrame = pos * offsetCF
                                end
                            end
                        end
                        
                        if tHRP.Position.Y < HRP.Position.Y + 4 and tChar.Parent == workspace then
                            local t_start = tick()
                            ...
                            if tHead then
                                repeat 
                                    task.wait() 
                                    pcall(function() SetNetOwner:FireServer(tHRP, tHRP.CFrame) end) 
                                until tHead:FindFirstChild("PartOwner") or (tick() - t_start > 1)
                            end
                            HRP.CFrame = pos
                            tHRP.CFrame = HRP.CFrame * offsetCF
                        end
...
Callback = function(v)
    ...
    task.spawn(function()
        ...
        while _G.applymethod and RunService.RenderStepped:Wait() do
            local tt = selectedKickPlayer
            if not tt or not tt.Character then continue end
            ...
            if HRP and tHRP then
                -- 1. Агрессивный захват контроля (Network Ownership)
                -- Без этого BodyPosition просто не будет работать на другого игрока
                pcall(function()
                    SetNetOwner:FireServer(tHRP, tHRP.CFrame)
                    DestroyLine:FireServer(tHRP)
                end)

                -- 2. Если дистанция слишком большая, контроль может не взяться.
                -- Делаем микро-телепорт цели к себе для "зацепа", если она далеко
                if (tHRP.Position - HRP.Position).Magnitude > 30 then
                    tHRP.CFrame = HRP.CFrame * CFrame.new(0, 5, 0)
                    task.wait(0.05)
                end

                -- 3. Создание объектов управления внутри цели
                if not kickbp or kickbp.Parent ~= tHRP then
                    if kickbp then kickbp:Destroy() end
                    kickbp = Instance.new("BodyPosition")
                    kickbp.Name = "GrabBP"
                    kickbp.Parent = tHRP
                    kickbp.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
                    kickbp.P = 50000 -- Увеличена мощность для подавления сопротивления
                    kickbp.D = 500
                end
                ...
                if not kickbg or kickbg.Parent ~= tHRP then
                    ...
                    kickbg.Parent = tHRP
                    kickbg.MaxTorque = Vector3.new(math.huge, math.huge, math.huge)
                    kickbg.CFrame = tHRP.CFrame
                end

                -- 4. Удержание над головой
                kickbp.Position = HRP.Position + upOffset
                
                -- 5. Принудительное обнуление импульса (чтобы не вырвался)
                tHRP.AssemblyLinearVelocity = Vector3.new(0, 0.1, 0) 
...
TargetGroup:AddToggle("DarkMatterKick", {
    Text = "Bomb Dark Matter",
    Default = false,
    Callback = function(Value)
        _G.DarkMatterActive = Value
        ...
        task.spawn(function()
            while _G.DarkMatterActive do
                local target = selectedKickPlayer 
                local myChar = Player.Character
                local myTorso = myChar and (myChar:FindFirstChild("HumanoidRootPart") or myChar:FindFirstChild("UpperTorso") or myChar:FindFirstChild("Torso"))
                
                if target and target.Character and myTorso then
                    local tChar = target.Character
                    local targetTorso = tChar:FindFirstChild("HumanoidRootPart") or tChar:FindFirstChild("UpperTorso") or tChar:FindFirstChild("Torso")

                    if targetTorso then
                        local spawnPos = myTorso.CFrame * CFrame.new(0, 20, 20)

                        pcall(function()
                            task.spawn(function()
                                SpawnRemote:InvokeServer("BombDarkMatter", spawnPos, Vector3.zero)
                            end)
                        end)
                        ...
                        if folder then
                            for _, item in ipairs(folder:GetChildren()) do
                                if item.Name == "BombDarkMatter" then
                                    local part = item:IsA("BasePart") and item or item.PrimaryPart or item:FindFirstChildWhichIsA("BasePart")
                                    if part then
                                        SetOwner:FireServer(part, targetTorso.CFrame)
                                        part.CFrame = targetTorso.CFrame
                                        part.AssemblyLinearVelocity = Vector3.zero
                                    end
                                end
                            end
...
TargetGroup:AddToggle("PlayerFlingBtn", {
	Text = "Ragdoll + Grab Target",
Default = false,
Callback = function(Value)
    getgenv().CombinedActive = Value
    
    local RS = game:GetService("ReplicatedStorage")
    local RunService = game:GetService("RunService")
    local lpName = game.Players.LocalPlayer.Name
    local toysFolder = workspace:WaitForChild(lpName .. "SpawnedInToys")
    ...
    if Value then
        if not selectedKickPlayer then
            Library:Notify("Select target first", 3)
            -- Здесь укажите путь к вашему тоглу, чтобы выключить его обратно
            return
        end

        -- Состояние для паллеты
        getgenv().PalletForRagdoll = nil
        
        -- 1. Функция спавна паллеты
        local function spawnNewPallet()
            if not getgenv().CombinedActive then return end
        ...
        -- 2. Отслеживание появления паллеты
        local connection
        connection = toysFolder.ChildAdded:Connect(function(child)
            if not getgenv().CombinedActive then 
                connection:Disconnect()
                return 
            end
            if child.Name ~= "PalletLightBrown" then return end
            ...
            -- Авто-респавн паллеты при удалении
            child.AncestryChanged:Connect(function()
                if not child.Parent and getgenv().CombinedActive then
                    task.wait(0.1)
                    spawnNewPallet()
                end
            end)
        ...
        -- 3. ОСНОВНОЙ ЦИКЛ (Grab + Ragdoll)
        task.spawn(function()
            local kickbp, kickbg
            spawnNewPallet()

            while getgenv().CombinedActive do
                local target = selectedKickPlayer
                ...
                if tHRP and myHRP then
                    -- ЛОГИКА GRAB (Удержание цели рядом)
                    if not kickbp or kickbp.Parent ~= tHRP then
                        if kickbp then kickbp:Destroy() end
                        kickbp = Instance.new("BodyPosition", tHRP)
                        kickbp.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
                        kickbp.P = 10000
                    end
                    if not kickbg or kickbg.Parent ~= tHRP then
                        if kickbg then kickbg:Destroy() end
                        kickbg = Instance.new("BodyGyro", tHRP)
                        kickbg.MaxTorque = Vector3.new(math.huge, math.huge, math.huge)
                    end

                    -- Позиция цели (в 5 метрах перед вами и чуть выше)
                    kickbp.Position = (myHRP.CFrame * CFrame.new(0, 5, -5)).Position
                    
                    -- Сетевой овнер (чтобы физика работала)
                    pcall(function()
                        SetNetOwner:FireServer(tHRP, tHRP.CFrame)
                        DestroyLine:FireServer(tHRP)
                    end)

                    -- ЛОГИКА RAGDOLL (Удары паллетой)
                    local pallet = getgenv().PalletForRagdoll
                    local sPart = pallet and pallet:FindFirstChild("SoundPart")
                    if sPart then
                        -- Быстро перемещаем паллету сверху на цель
                        sPart.CFrame = tHRP.CFrame * CFrame.new(0, 3, 0)
                        sPart.AssemblyLinearVelocity = Vector3.new(0, -250, 0)
                    end
                end
                RunService.RenderStepped:Wait()
            end
            ...
            local p = getgenv().PalletForRagdoll
            if p then pcall(function() DestroyToy:FireServer(p) end) end
        end)

    else
        getgenv().CombinedActive = false
    end
end
})
		...
		if on then
			...
			notifyConnection = PS.PlayerAdded:Connect(function(newPlayer)
				...
				if detected then
					...
					sound.SoundId = "8679659744://8679659744"
					sound.Volume = 2
					sound:Play()
...
MiscGroup:AddToggle("AntiDetect", {
...
MiscGroup:AddButton("Break Barrier", function()
    local pos = HRP.CFrame
    local t = tick()
...
local function playKickSound()
	...
	s.SoundId = "rbxassetid://8679659744" -- Bell (Deltarune)
	s.Volume = 1
	s.PlayOnRemove = true
...
local jerkOffAnimId = "rbxassetid://8679659744" -- анимация
